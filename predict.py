...
BASE_MODEL_URL = (
    "https://weights.replicate.delivery/default/SG161222--RealVisXL_V3.0-11ee564ebf4bd96d90ed5d473cb8e7f2e6450bcf.tar"
)
...
class Predictor(BasePredictor):
    def setup(self) -> None:
        ...
        self.safety_checker = StableDiffusionSafetyChecker.from_pretrained(SAFETY_CACHE, torch_dtype=torch.float16).to(
            "cuda"
        )
    ...
    def predict(
        self,
        input_image: Path = Input(description="The input image, for example a photo of your face."),
        input_image2: Path = Input(description="Additional input image (optional)", default=None),
        input_image3: Path = Input(description="Additional input image (optional)", default=None),
        input_image4: Path = Input(description="Additional input image (optional)", default=None),
        prompt: str = Input(
        ...
        num_steps: int = Input(description="Number of sample steps", default=20, ge=1, le=100),
        style_strength_ratio: float = Input(description="Style strength (%)", default=20, ge=15, le=50),
        num_outputs: int = Input(description="Number of output images", default=1, ge=1, le=4),
        ...
        if image_token_id not in input_ids:
            raise ValueError(f"Cannot find the trigger word '{self.pipe.trigger_word}' in text prompt!")

        if input_ids.count(image_token_id) > 1:
            raise ValueError(f"Cannot use multiple trigger words '{self.pipe.trigger_word}' in text prompt!")
        ...
        if negative_prompt:
            ...
            if image_token_id in negative_prompt_ids:
                raise ValueError(f"Cannot use trigger word '{self.pipe.trigger_word}' in negative prompt!")
    ...
    def run_safety_checker(self, image):
        ...
        image, has_nsfw_concept = self.safety_checker(
            images=np_image, clip_input=safety_checker_input.pixel_values.to(torch.float16)
        )
